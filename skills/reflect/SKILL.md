...
3. **Spawn one read-only reviewer — different weights by default.** A same-model judge shares your blind spots, so **by default** route the brief through `./eval.sh` — a headless judge of genuinely different weights (the independent engine, `opencode`, on its own default model), the sharper break from self-grading. Opt back to a same-model sub-agent on *this* engine (the `Explore` subagent_type, or `general-purpose` if it needs to run code to verify) via the Agent tool only when `$ARGUMENTS` or `$REFLECT_ENGINE` asks for it with `same`. `$REFLECT_ENGINE`/`$REFLECT_MODEL` override the engine and model otherwise. Give the reviewer everything it needs and nothing about the journey:
   ...
   - **Dispatch:** *independent engine (default)* → write the brief to a file under `/tmp` (e.g. `/tmp/reflect-brief.md`) and run `./eval.sh --cwd <repo> --prompt-file /tmp/reflect-brief.md`; it runs headless and prints the review to stdout — read it from there. *Same-model (`same`)* → spawn via the Agent tool; its report returns to you.
   - **Its permissions are derived from yours, with two deliberate deviations.** The rules come from `~/.claude/settings.json` — the one place they live — translated by `./reviewer-permissions.sh`, because opencode cannot read that file. The reviewer inherits your denies (secrets, force-push, destructive commands). It is **stricter** on edits: file-editing tools are stripped from its toolset outright. It is **weaker** in exactly one place: it may read tracked `.env.*.example` templates, which you cannot — they are placeholders it must see to judge a diff, and Claude's deny is absolute so it cannot carry that exception. Bash is **not** blocked in either engine, so this is a speed bump, not a sandbox — the mandate is what holds the reviewer read-only, exactly as for the Explore/general-purpose sub-agents.
   - **Watch stderr for a `WARNING — the reviewer was DENIED …`.** It means a tool call was refused and the review may be **partial**; a review that never ran must never be relayed to the user as "no findings". If the denied path is a tracked file the reviewer legitimately needs, add it via `--allow-read` in `./reviewer-permissions.sh`.
