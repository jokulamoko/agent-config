...
3. **Spawn one read-only reviewer — a different model by default.** A same-model judge shares your blind spots, so reflect routes the brief to a **different Claude model** — the sharper break from self-grading. Identify the model you're running now and pick the reviewer by this map:
   - **opus → sonnet**
   - **sonnet → opus**
   - **fable → opus**
   - anything else → opus

   Spawn the reviewer via the Agent tool with that `model` override — the `Explore` subagent_type (read-only), or `general-purpose` if it needs to run code to verify. `$REFLECT_MODEL`, if set, overrides the chosen reviewer model. Give the reviewer everything it needs and nothing about the journey:
   ...
   - **The mandate:** *read-only*. It investigates and reports — it does not edit, fix, or commit anything. The `Explore` subagent has no file-editing tools; a `general-purpose` reviewer is held read-only by this mandate alone.
